...
#'
#' @export
sr_awesomecv <- function(..., latex_engine = "xelatex", page_total = FALSE,
                      show_footer = TRUE) {
  template <- system.file("rmarkdown", "templates", "sr_awesomecv",
                          "resources", "sr_awesome-cv.tex",
                          package = "srvitae"
  )
  set_entry_formats(sr_awesome_cv_entries)
  copy_supporting_files("sr_awesomecv")
  pandoc_vars <- list()
...
sr_awesome_cv_entries <- new_entry_formats(
